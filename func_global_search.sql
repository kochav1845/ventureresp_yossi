CREATE OR REPLACE FUNCTION public.global_search(search_query text, max_per_category integer DEFAULT 6)
 RETURNS TABLE(category text, item_id text, title text, subtitle text, meta_line text, route text, relevance real)
 LANGUAGE plpgsql
