...
export function useInvoiceComparison(onDataRefresh?: () => void) {
  ...
  const runComparison = useCallback(async (key: string, startDate: string, endDate: string) => {
    ...
    try {
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/acumatica-get-invoice-count`;
    ...
    const checkRunningJobs = async () => {
      try {
        ...
        for (const job of runningJobs) {
          ...
          const start = job.start_date.split('T')[0].split(' ')[0];
          const end = job.end_date.split('T')[0].split(' ')[0];
          ...
          if (start === end) {
            ...
            if (sy === ey && start.endsWith('-01')) {
              const lastDay = new Date(ey, em, 0).getDate();
              if (end.endsWith(`-${String(lastDay).padStart(2, '0')}`)) {
                key = `${sy}-${String(sm).padStart(2, '0')}`;
              }
            }
          }
          if (!key) continue;

          pollJobStatus(job.id, key, start, end);
        }
      } catch {
      }
    };
  ...
  const runFetch = useCallback(async (key: string, startDate: string, endDate: string) => {
    ...
    try {
      ...
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/acumatica-invoice-date-range-sync`;
      const { data: { session } } = await supabase.auth.getSession();
