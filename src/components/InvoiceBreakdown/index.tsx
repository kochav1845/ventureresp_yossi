...
export default function InvoiceBreakdown() {
  ...
  const handleExportCSV = () => {
    const rows = filteredMonths.map(m => ({
      ...
      'Open Count': m.open_count,
      'Open Amount': m.open_amount,
      'Open Balance': m.open_balance,
      'Closed Count': m.closed_count,
      'Closed Amount': m.closed_amount,
      'Balanced Count': m.balanced_count,
      'Balanced Amount': m.balanced_amount,
      'Canceled Count': m.canceled_count,
      'Canceled Amount': m.canceled_amount,
      'Voided Count': m.voided_count,
      'Voided Amount': m.voided_amount,
      'Credit Hold Count': m.credit_hold_count,
      'Credit Hold Amount': m.credit_hold_amount,
      'On Hold Count': m.on_hold_count,
      'On Hold Amount': m.on_hold_amount,
