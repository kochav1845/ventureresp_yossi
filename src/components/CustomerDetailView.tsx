  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-8 mb-6">
        ...
        {/* Invoice Color Status Summary */}
        <div className="mb-6 p-6 bg-gradient-to-r from-slate-50 to-gray-100 border-2 border-gray-300 rounded-lg shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-gray-900 mb-1">Invoice Status Summary</h3>
              <p className="text-sm text-gray-600">Click a color to view those invoices</p>
            </div>
                ...
                >
                  <div className={`w-20 h-20 rounded-lg bg-red-500 border-4 flex items-center justify-center shadow-lg ${
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-white font-semibold">RED</div>
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 mt-1 font-medium">Won't Pay</p>
                </button>
              )}
                ...
                >
                  <div className={`w-20 h-20 rounded-lg bg-yellow-400 border-4 flex items-center justify-center shadow-lg ${
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-900 font-semibold">YELLOW</div>
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 mt-1 font-medium">Will Take Care</p>
                </button>
              )}
                ...
                >
                  <div className={`w-20 h-20 rounded-lg bg-green-500 border-4 flex items-center justify-center shadow-lg ${
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-white font-semibold">GREEN</div>
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 mt-1 font-medium">Will Pay</p>
                </button>
              )}
              {invoiceColorCounts.total === 0 && (
                <div className="text-center py-4">
                  <p className="text-gray-500 font-medium">No open invoices with color status</p>
                </div>
              )}
              {invoiceColorCounts.total > 0 && invoiceColorCounts.red === 0 && invoiceColorCounts.yellow === 0 && invoiceColorCounts.green === 0 && (
                <div className="text-center py-4">
                  <p className="text-gray-500 font-medium">Open invoices have no color status assigned yet</p>
                  <p className="text-xs text-gray-400 mt-1">Go to Invoices page to assign colors</p>
                </div>
              )}
            </div>
          </div>
          {invoiceColorCounts.total > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-300">
              <p className="text-sm text-gray-600">
                <span className="font-semibold">{invoiceColorCounts.total}</span> total open invoice{invoiceColorCounts.total !== 1 ? 's' : ''} •
                <span className="ml-2">{invoiceColorCounts.red + invoiceColorCounts.yellow + invoiceColorCounts.green} with color status assigned</span>
              </p>
            </div>
          )}
        </div>
      ...
      {hasPermission(PERMISSION_KEYS.CUSTOMERS_EDIT, 'edit') && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center mb-4">
            <Tag className="w-5 h-5 text-blue-600 mr-2" />
            <h2 className="text-lg font-semibold text-gray-900">Analytics Exclusion Settings</h2>
          </div>
          <p className="text-sm text-gray-600 mb-4">
            Exclude this customer from specific analytics reports. Useful for filtering out test accounts or internal customers.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
              <input
                type="checkbox"
                checked={customer?.exclude_from_payment_analytics || false}
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    await logActivity(
                      ...
                      customerId,
                      {
                        field: 'exclude_from_payment_analytics',
                        value: newValue,
                        customer_name: customer.customer_name
                      }
                    );
              ...
              <span className="text-sm font-medium text-gray-700">
                Exclude from Payment Analytics
              </span>
            </label>
            ...
            <label className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
              <input
                type="checkbox"
                checked={customer?.exclude_from_invoice_analytics || false}
                onChange={async (e) => {
                  const newValue = e.target.checked;
                  try {
                    const { error } = await supabase
                    ...
                    await logActivity(
                      ...
                      customerId,
                      {
                        field: 'exclude_from_invoice_analytics',
                        value: newValue,
                        customer_name: customer.customer_name
                      }
                    );
              ...
              />
              <span className="text-sm font-medium text-gray-700">
                Exclude from Invoice Analytics
              </span>
            </label>
            ...
            <label className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
              <input
                type="checkbox"
                checked={customer?.exclude_from_customer_analytics || false}
                onChange={async (e) => {
                  const newValue = e.target.checked;
                  try {
                    const { error } = await supabase
                    ...
                    await logActivity(
                      ...
                      customerId,
                      {
                        field: 'exclude_from_customer_analytics',
                        value: newValue,
                        customer_name: customer.customer_name
                      }
                    );
