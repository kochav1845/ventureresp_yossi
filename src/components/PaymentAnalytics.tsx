...
export default function PaymentAnalytics({ onBack }: PaymentAnalyticsProps) {
  ...
  // Sidebar customer exclusion (per-user, saved in default filters)
  const [sidebarExcludedCustomers, setSidebarExcludedCustomers] = useState<string[]>(() => c?.sidebarExcludedCustomers ?? []);
  const [tempSidebarExcludedCustomers, setTempSidebarExcludedCustomers] = useState<string[]>(() => c?.sidebarExcludedCustomers ?? []);
  ...
  }, [payments, searchTerm, sortField, sortDirection, filterStatus, filterType, filterPaymentMethod, filterInvoicePeriod, selectedCustomers, selectedDate, excludedCustomerIds, sidebarExcludedCustomers]);
  ...
  const filterAndSortPayments = () => {
    ...
    if (sidebarExcludedCustomers.length > 0) {
      const excludeSet = new Set(sidebarExcludedCustomers);
      filtered = filtered.filter(p => !excludeSet.has(p.customer_id));
    }
  ...
  const filteredCustomerOptions = useMemo(() => {
    if (!customerFilterSearch) return [];
  ...
  const applyFilters = () => {
    ...
    setSidebarExcludedCustomers(tempSidebarExcludedCustomers);
  };
  ...
  const clearFilters = () => {
    ...
    setDateFrom('');
    setDateTo('');
    setSidebarExcludedCustomers([]);
    setTempSidebarExcludedCustomers([]);
  ...
  const saveAsDefaultFilters = async () => {
    ...
    const filters = {
      ...
      filterPaymentMethod: tempFilterPaymentMethod,
      filterInvoicePeriod: tempFilterInvoicePeriod,
    ...
    await supabase.from('user_analytics_default_filters').upsert({
      user_id: user.id,
      page: 'payment_analytics',
      filters,
      excluded_customers: tempSidebarExcludedCustomers,
  ...
  const applyDefaultFilters = async () => {
    ...
    if (data) {
      ...
      setSidebarExcludedCustomers(data.excluded_customers || []);
      setDefaultFiltersActive(true);
    }
  };
  ...
  const removeDefaultFilters = async () => {
    if (!user) return;
    await supabase.from('user_analytics_default_filters').delete().eq('user_id', user.id).eq('page', 'payment_analytics');
  ...
  const addSidebarExcludedCustomer = (customerId: string) => {
    setTempSidebarExcludedCustomers(prev => prev.includes(customerId) ? prev : [...prev, customerId]);
    setExcludeSearchTerm('');
  };

  const removeSidebarExcludedCustomer = (customerId: string) => {
    setTempSidebarExcludedCustomers(prev => prev.filter(c => c !== customerId));
  };
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {customerFilterSearch && (
                      <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg bg-white">
                        {filteredCustomerOptions.length === 0 ? (
                          <p className="text-xs text-gray-400 p-3 text-center">No customers match "{customerFilterSearch}"</p>
                        ) : (
                    ...
                    {tempSidebarExcludedCustomers.length > 0 && (
                      <div className="mt-2 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-semibold text-red-600 uppercase tracking-wider">Excluded Customers</span>
                          <button
                            onClick={() => setTempSidebarExcludedCustomers([])}
                        ...
                        <div className="flex flex-wrap gap-1.5 p-2 bg-red-50/50 border border-red-200 rounded-lg">
                          {tempSidebarExcludedCustomers.map(id => {
                            const cust = uniqueCustomers.find(c => c.id === id);
                            const displayName = cust?.name || id;
                            return (
                              <span
                                key={id}
                                className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-800 rounded-md text-xs font-medium max-w-full"
                              >
                                <Ban className="w-3 h-3 flex-shrink-0" />
                                <span className="truncate">{displayName}</span>
                                <button
                                  onClick={() => removeSidebarExcludedCustomer(id)}
                                  className="flex-shrink-0 hover:text-red-600 ml-0.5"
                                >
                                  <X className="w-3 h-3" />
                                </button>
                              </span>
                  ...
                  {hasActiveFilters && (
                    <div className="bg-amber-50 border border-amber-300 rounded-lg p-2 space-y-1">
                      ...
                      {sidebarExcludedCustomers.length > 0 && (
                        <p className="text-[10px] text-amber-600">
                          {sidebarExcludedCustomers.length} customer{sidebarExcludedCustomers.length !== 1 ? 's' : ''} excluded
                        </p>
                      )}
                    </div>
                  )}
