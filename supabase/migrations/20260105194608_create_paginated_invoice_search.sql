/*
  # Create Paginated Invoice Search

  1. New Functions
    - `search_invoices_paginated` - Returns paginated invoice search results (50 at a time)
    - `search_invoices_count` - Returns total count of matching invoices

  2. Features
    - Server-side search (reference, customer, description, order)
    - Server-side filtering (status, customer, balance, color, dates)
    - Server-side sorting (date, amount, balance, reference, status, etc.)
    - Pagination support (LIMIT/OFFSET)
    - Exact match optimization for reference numbers

  3. Performance
    - Uses indexes on key columns
    - Returns only 50 records per page
    - Exact match uses direct index lookup
    - Pattern matching limited for performance
*/
...
-- Function to get count of matching invoices
...
BEGIN
  ...
  IF search_term IS NOT NULL AND search_term != '' THEN
    ...
    RETURN result_count;
  ELSE
    -- No search term, just count with filters
    SELECT COUNT(*) INTO result_count
    FROM acumatica_invoices i
    WHERE 
      (status_filter IS NULL OR status_filter = 'all' OR i.status = status_filter)
...
COMMENT ON FUNCTION search_invoices_paginated IS 
  'Returns paginated invoice search results (50 at a time) with all filters and sorting on server side';

COMMENT ON FUNCTION search_invoices_count IS 
  'Returns total count of invoices matching search criteria for pagination';

GRANT EXECUTE ON FUNCTION search_invoices_paginated TO authenticated;
