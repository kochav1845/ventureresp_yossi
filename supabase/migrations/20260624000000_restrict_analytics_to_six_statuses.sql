/*
  ...
  3. Notes
    - This excludes "On Hold" and "Scheduled" docs from TOTALS only.
    ...
    - NOTE: this migration was already applied to production directly on 2026-06-24
      via the SQL editor; this file restores the repo record after it was dropped
      by a later Bolt snapshot commit.
*/
...
BEGIN
  IF p_period_type = 'daily' THEN
    ...
    WITH base AS (
      SELECT i.date::date AS inv_date, i.type, i.status, i.amount, i.balance, i.customer
    ...
    main_agg AS (
      ...
      FROM base GROUP BY inv_date
    ),
    type_agg AS (
      SELECT inv_date, jsonb_object_agg(type, jsonb_build_object('count', cnt, 'amount', amt)) AS type_amounts
      FROM (SELECT inv_date, type, COUNT(*) AS cnt, SUM(amount) AS amt FROM base GROUP BY inv_date, type) t
      GROUP BY inv_date
    ),
    status_agg AS (
      SELECT inv_date, jsonb_object_agg(status, jsonb_build_object('count', cnt, 'amount', amt)) AS status_breakdown
      FROM (SELECT inv_date, status, COUNT(*) AS cnt, SUM(amount) AS amt FROM base GROUP BY inv_date, status) s
      GROUP BY inv_date
    )
    INSERT INTO cached_invoice_analytics (
      organization_id, period_type, year, month, day, date,
      total_amount, total_balance, total_open_balance, invoice_count, unique_customer_count,
      invoice_only_amount, invoice_only_count, credit_memo_amount, credit_memo_count,
      debit_memo_amount, debit_memo_count, open_invoice_balance, open_invoice_count,
      balanced_invoice_balance, balanced_invoice_count, open_cm_balance, open_cm_count,
      open_dm_balance, open_dm_count, type_amounts, status_breakdown, calculated_at, updated_at
    )
    SELECT p_org_id, 'daily',
      EXTRACT(YEAR FROM m.inv_date)::integer, EXTRACT(MONTH FROM m.inv_date)::integer, EXTRACT(DAY FROM m.inv_date)::integer, m.inv_date,
      m.total_amount, m.total_balance, m.total_open_balance, m.invoice_count, m.unique_customer_count,
      m.invoice_only_amount, m.invoice_only_count, m.credit_memo_amount, m.credit_memo_count,
      m.debit_memo_amount, m.debit_memo_count, m.open_invoice_balance, m.open_invoice_count,
      m.balanced_invoice_balance, m.balanced_invoice_count, m.open_cm_balance, m.open_cm_count,
      m.open_dm_balance, m.open_dm_count, COALESCE(ta.type_amounts, '{}'), COALESCE(sa.status_breakdown, '{}'), now(), now()
    FROM main_agg m
    ...
    DO UPDATE SET
      total_amount = EXCLUDED.total_amount, total_balance = EXCLUDED.total_balance, total_open_balance = EXCLUDED.total_open_balance,
      invoice_count = EXCLUDED.invoice_count, unique_customer_count = EXCLUDED.unique_customer_count,
      invoice_only_amount = EXCLUDED.invoice_only_amount, invoice_only_count = EXCLUDED.invoice_only_count,
      credit_memo_amount = EXCLUDED.credit_memo_amount, credit_memo_count = EXCLUDED.credit_memo_count,
      debit_memo_amount = EXCLUDED.debit_memo_amount, debit_memo_count = EXCLUDED.debit_memo_count,
      open_invoice_balance = EXCLUDED.open_invoice_balance, open_invoice_count = EXCLUDED.open_invoice_count,
      balanced_invoice_balance = EXCLUDED.balanced_invoice_balance, balanced_invoice_count = EXCLUDED.balanced_invoice_count,
      open_cm_balance = EXCLUDED.open_cm_balance, open_cm_count = EXCLUDED.open_cm_count,
      open_dm_balance = EXCLUDED.open_dm_balance, open_dm_count = EXCLUDED.open_dm_count,
      type_amounts = EXCLUDED.type_amounts, status_breakdown = EXCLUDED.status_breakdown,
  ...
  ELSIF p_period_type = 'monthly' THEN
    INSERT INTO cached_invoice_analytics (
      ...
      debit_memo_amount, debit_memo_count, open_invoice_balance, open_invoice_count,
      balanced_invoice_balance, balanced_invoice_count, open_cm_balance, open_cm_count,
      open_dm_balance, open_dm_count, calculated_at, updated_at
    )
    SELECT p_org_id, 'monthly', p_target_year, EXTRACT(MONTH FROM i.date::date)::integer, NULL::integer, NULL::date,
      COALESCE(SUM(i.amount), 0), COALESCE(SUM(i.balance), 0),
      COALESCE(SUM(CASE WHEN i.status IN ('Open', 'Balanced', 'Credit Hold') THEN i.balance ELSE 0 END), 0),
      COUNT(*)::integer, COUNT(DISTINCT i.customer)::integer,
      COALESCE(SUM(CASE WHEN i.type = 'Invoice' THEN i.amount ELSE 0 END), 0), COUNT(CASE WHEN i.type = 'Invoice' THEN 1 END)::integer,
      COALESCE(SUM(CASE WHEN i.type = 'Credit Memo' THEN i.amount ELSE 0 END), 0), COUNT(CASE WHEN i.type = 'Credit Memo' THEN 1 END)::integer,
      COALESCE(SUM(CASE WHEN i.type = 'Debit Memo' THEN i.amount ELSE 0 END), 0), COUNT(CASE WHEN i.type = 'Debit Memo' THEN 1 END)::integer,
    ...
    SELECT p_org_id, 'monthly', p_target_year, m.month_num, NULL, NULL,
    ...
    WHERE NOT EXISTS (
      ...
      WHERE c.organization_id = p_org_id AND c.period_type = 'monthly' AND c.year = p_target_year AND c.month = m.month_num
    )
    ON CONFLICT (organization_id, period_type, COALESCE(year, 0), COALESCE(month, 0), COALESCE(day, 0)) DO NOTHING;

  ELSIF p_period_type = 'yearly' THEN
    SELECT EXTRACT(YEAR FROM MIN(date))::integer, EXTRACT(YEAR FROM MAX(date))::integer
    INTO v_min_year, v_max_year
    FROM acumatica_invoices
    WHERE organization_id = p_org_id AND date IS NOT NULL
    ...
    IF v_min_year IS NOT NULL THEN
      FOR v_year IN v_min_year..v_max_year LOOP
        ...
        SELECT p_org_id, 'yearly', v_year, NULL::integer, NULL::integer, NULL::date,
          COALESCE(SUM(i.amount), 0), COALESCE(SUM(i.balance), 0),
          COALESCE(SUM(CASE WHEN i.status IN ('Open', 'Balanced', 'Credit Hold') THEN i.balance ELSE 0 END), 0),
          COUNT(*)::integer, COUNT(DISTINCT i.customer)::integer,
          COALESCE(SUM(CASE WHEN i.type = 'Invoice' THEN i.amount ELSE 0 END), 0), COUNT(CASE WHEN i.type = 'Invoice' THEN 1 END)::integer,
  ...
  RETURN jsonb_build_object('success', true, 'period_type', p_period_type, 'year', p_target_year, 'month', p_month, 'org_id', p_org_id, 'rows_affected', v_rows_affected);
END;
$$;
...
BEGIN
  ...
  SELECT fp.yr,
    ...
    COALESCE(SUM(fp.amt), 0), COUNT(*), COUNT(DISTINCT fp.cid),
