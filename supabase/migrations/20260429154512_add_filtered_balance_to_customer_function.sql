/*
  # Add filtered balance columns to get_customers_with_balance

  1. Changes
    - Drops and recreates the function with 2 new output columns:
      `filtered_gross_balance` and `filtered_invoice_count`
    - When date range or days-overdue filters are active, these columns only sum
      invoices matching those filters
    - When no such filters are active, filtered columns match the unfiltered totals
    - Stats cards in the Customer Dashboard can use filtered values for accurate totals

  2. Important Notes
    - Same input signature; 2 new output columns added
    - All existing callers continue to work; new columns are additive
    - The unfiltered `gross_balance`, `calculated_balance`, `open_invoice_count` columns
      remain unchanged for backward compatibility
*/

DROP FUNCTION IF EXISTS public.get_customers_with_balance(text,text,text,text,text,integer,integer,timestamptz,timestamptz,text,numeric,numeric,integer,integer,numeric,numeric,boolean,text,boolean,integer,integer,boolean);
...
v_has_filter := (p_date_from IS NOT NULL OR p_date_to IS NOT NULL
...
-- Filtered balance: only invoices matching date + days-overdue filters
...
cb.max_overdue_days,
cacd.avg_days,
cb.filtered_gross_bal,
cb.filtered_inv_count
