/*
  # Add filtered net balance to get_customers_with_balance

  1. Changes
    - Adds `filtered_net_balance` output column that subtracts credit memos
      from the filtered gross balance
    - This allows the stats cards to show accurate Total Outstanding Balance
      that accounts for credit memos as a negative

  2. Important Notes
    - Drops and recreates function (return type changed)
    - All prior columns remain unchanged
    - `filtered_net_balance` = `filtered_gross_balance` - credit memo balances
*/
...
-- Filtered gross: only invoices/debit memos matching date + days-overdue filters
