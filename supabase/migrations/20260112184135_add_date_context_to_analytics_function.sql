/*
  # Add Date Context to Analytics Function

  1. Changes
    - Add p_date_context parameter to get_customer_analytics function
    - Apply date filtering based on context (invoice_date, balance_date, customer_added)
    - Match the same logic as get_customers_with_balance for consistency

  2. Notes
    - Ensures analytics match the customer list filtering
*/
...
-- Create the updated analytics function with date context
...
BEGIN
  ...
  filtered_customers AS (
    ...
    FROM acumatica_customers c
    INNER JOIN date_filtered_customers dfc ON c.customer_id = dfc.customer
    WHERE
      (p_search IS NULL OR (
