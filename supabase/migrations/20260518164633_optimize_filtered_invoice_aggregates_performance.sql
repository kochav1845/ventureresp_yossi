/*
  # Optimize get_filtered_invoice_aggregates for performance

  1. Changes
    - Rewrite as pure SQL function for better query planning
    - Use date_part instead of EXTRACT for consistency
    - Add index on (status, date) to support the common yearly scan
    - Pre-filter with status != 'On Hold' to use partial index

  2. Notes
    - The yearly aggregation was timing out on 104K rows
    - SQL functions allow the planner to inline the query
*/

CREATE INDEX IF NOT EXISTS idx_invoices_not_on_hold_date_year
  ON acumatica_invoices (date)
