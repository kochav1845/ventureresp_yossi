/*
  # Fix get_payments_with_applications timeout

  Convert to SECURITY DEFINER with explicit org filter to avoid
  per-row RLS evaluation on large payment tables.
*/
...
BEGIN
  ...
  WHERE p.organization_id = v_org_id
    AND public.payment_effective_date(p.doc_date, p.application_date) >= p_start_date
