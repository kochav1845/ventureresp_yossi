/*
  # Add Advanced Filter Support to Analytics Function

  1. Changes
    - Add min_balance, max_balance, min_open_invoices, max_open_invoices parameters
    - Support balance filtering (positive, negative, zero, or all)
    - Ensure analytics respect ALL filter criteria

  2. Purpose
    - Make analytics match the exact filter state
    - Support advanced filtering like min/max balance and invoice counts
    - Ensure analytics always reflect filtered data, not just basic filters
*/

-- Drop existing function
DROP FUNCTION IF EXISTS get_customer_analytics(text, text, text, timestamptz, timestamptz, text[]);

-- Create the updated analytics function with all filter support
    ...
    SELECT
      ...
      COALESCE(
        (SELECT SUM(i.balance)
         ...
         AND (p_date_from IS NULL OR i.invoice_date >= p_date_from::date)
         AND (p_date_to IS NULL OR i.invoice_date <= p_date_to::date)
        ), 0
