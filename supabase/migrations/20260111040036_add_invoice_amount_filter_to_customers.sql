/*
  # Add Individual Invoice Amount Filter to Customers Function

  1. Changes
    - Adds p_min_invoice_amount parameter for minimum invoice amount filtering
    - Adds p_max_invoice_amount parameter for maximum invoice amount filtering
    - Only includes invoices that fall within the amount range when calculating customer balances
    - This filters by individual invoice amounts, not customer total balance

  2. Benefits
    - Allows finding customers who have invoices within a specific amount range
    - Example: Find customers with invoices between $30-$2000
    - Improves performance by filtering at database level
*/

-- Drop existing function
DROP FUNCTION IF EXISTS get_customers_with_balance(TEXT, TEXT, TEXT, TEXT, TEXT, INT, INT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, NUMERIC, NUMERIC, INT, INT);

-- Create updated version with invoice amount filter parameters
...
BEGIN
  ...
  WITH customer_balances AS (
    ...
    WHERE i.balance > 0
      -- Apply invoice amount filter at the invoice level
  ...
  date_filtered_customers AS (
    ...
    WHERE
      ...
      -- Also apply invoice amount filter here for date filtering
      AND (p_min_invoice_amount IS NULL OR i.balance >= p_min_invoice_amount)
      AND (p_max_invoice_amount IS NULL OR i.balance <= p_max_invoice_amount)
  )
  SELECT
  ...
  WHERE
    ...
    -- Balance filtering at database level (filters by customer's TOTAL balance)
    AND (
    ...
    -- Invoice count filtering at database level
    ...
    -- Only show customers who have invoices matching the amount filter (if specified)
    AND (
