/*
  # Add type column and include Balanced status in invoice functions

  1. Changes to `get_customer_invoices_advanced`
    - Added `type` (text) to return columns
    - 'open' filter now includes both Open and Balanced statuses
    - Credit Memo and Credit WO amounts/balances returned as negative values

  2. Changes to `get_customer_invoices_advanced_count`
    - 'open' filter now includes both Open and Balanced statuses
    - Credit memos counted with negative balance for accurate totals

  3. Why
    - Users need to see document type (Invoice, Credit Memo, Debit Memo) in the list
    - Balanced invoices should appear alongside Open invoices
    - Credit memos must display as negative amounts for clarity
*/
