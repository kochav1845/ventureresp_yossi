/*
  # Add days overdue filter to customer invoices advanced functions

  1. Modified Functions
    - `get_customer_invoices_advanced` - Added `p_min_days_overdue` and `p_max_days_overdue` parameters
      to filter invoices by their days overdue (calculated from invoice date)
    - `get_customer_invoices_advanced_count` - Added matching parameters for consistent filtering

  2. Changes
    - New optional parameters: p_min_days_overdue (integer), p_max_days_overdue (integer)
    - Days overdue calculated as CURRENT_DATE - i.date (using invoice date, not due date)
    - Only applies to invoices with a positive balance and a non-null date

  3. Important Notes
    - Both functions must be dropped first due to parameter default changes
    - Days overdue uses invoice date to match the customer list page behavior
*/

DROP FUNCTION IF EXISTS public.get_customer_invoices_advanced(text, text, date, date, numeric, numeric, text, text, text, text, integer, integer, boolean);

CREATE OR REPLACE FUNCTION public.get_customer_invoices_advanced(
  p_customer_id text,
  p_filter text,
  p_date_from date,
  p_date_to date,
...
DROP FUNCTION IF EXISTS public.get_customer_invoices_advanced_count(text, text, date, date, numeric, numeric, text, text, boolean);

CREATE OR REPLACE FUNCTION public.get_customer_invoices_advanced_count(
  p_customer_id text,
  p_filter text,
  p_date_from date,
  p_date_to date,
