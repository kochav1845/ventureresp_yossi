/*
  # Create Customer Analytics Timeline Function

  1. Purpose
    - Provides time-series data for invoices opened, payments made, and balance owed
    - Supports custom date ranges with automatic grouping (daily/weekly/monthly)
    - Aggregates data across all customers for overview analytics

  2. Returns
    - date: The date point
    - invoices_opened: Count of invoices created on that date
    - invoice_amount: Total amount of invoices created
    - payments_made: Count of payments made
    - payment_amount: Total payment amount
    - balance_owed: Running balance at that point in time
*/
...
BEGIN
  ...
  invoices_by_period AS (
    SELECT
      ...
      SUM(i.doc_balance) as invoice_total
