/*
  # Drastically Optimize Invoice Search Count to Prevent Timeouts

  1. Problem
    - search_invoices_count still timing out on large result sets
    - ILIKE pattern matching across multiple columns is slow
    - COUNT(*) on subquery still scans too many rows
    
  2. Solution
    - Add aggressive statement_timeout (2 seconds max)
    - Reduce default max_count to 1000 (from 10000)
    - Simplify search logic to prioritize speed over accuracy
    - Return estimate when exact count would be too slow
    - Use indexes more aggressively
    
  3. Changes
    - Set statement_timeout to 2 seconds within function
    - Reduce max_count default to 1000
    - Optimize ILIKE searches to use fewer columns
    - Add EXPLAIN ANALYZE safeguards
*/

-- Drop and recreate with aggressive optimizations
...
DECLARE
  ...
  old_timeout TEXT;
BEGIN
  -- Set aggressive timeout (2 seconds max) and save old value
  SHOW statement_timeout INTO old_timeout;
  ...
  IF search_term IS NOT NULL AND search_term != '' THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- If timeout, return max_count to indicate "many results"
        RETURN max_count;
    END;
    
    -- For pattern matching, just return estimated max instead of slow ILIKE
    -- This prevents timeouts at the cost of showing "~1000" instead of exact count
    -- The actual results will still be accurate, just the count is approximate
    BEGIN
      -- Try a much faster approach: just check if ANY results exist
      PERFORM 1
      FROM acumatica_invoices i
      WHERE 
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- If timeout or error, return max_count
        RETURN max_count;
    END;
  ELSE
    -- No search term, count with filters only
    BEGIN
      SELECT COUNT(*) INTO result_count
      FROM (
        SELECT 1
...
COMMENT ON FUNCTION search_invoices_count IS 
  'Returns fast approximate count of invoices (max 1000, 2s timeout). For pattern searches, returns max_count if any results exist to prevent timeouts. Exact count only for filtered queries.';
