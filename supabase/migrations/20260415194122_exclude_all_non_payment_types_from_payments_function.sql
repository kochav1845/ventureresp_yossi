/*
  # Exclude all non-payment types from get_payments_with_applications

  1. Changes
    - Update get_payments_with_applications to exclude Credit Memo, Balance WO, Cash Sale, Cash Return
    - Previously only excluded Credit Memo; now consistent with get_filtered_payment_aggregates
    - Always exclude these types regardless of p_exclude_credit_memos parameter
    - The p_type parameter still allows explicit type filtering

  2. Why
    - Credit memos and other non-payment types should never appear in payment analytics
    - Makes exclusion consistent across all payment analytics functions
*/
...
WHERE public.payment_effective_date(p.doc_date, p.application_date) >= p_start_date
  ...
  AND (
    ...
    OR (p_type IS NULL AND p.type NOT IN ('Credit Memo', 'Balance WO', 'Cash Sale', 'Cash Return'))
  )
