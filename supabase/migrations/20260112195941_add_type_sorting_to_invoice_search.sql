/*
  # Add Type Sorting to Invoice Search

  1. Changes
    - Add 'type' sorting option to `search_invoices_paginated` function
    - Sort entire database by type first, then paginate (like status sorting)
    - Supports both ascending and descending order
    
  2. Behavior
    - sort_by = 'type', sort_order = 'asc': Credit Memo, Debit Memo, Invoice, etc. (A-Z)
    - sort_by = 'type', sort_order = 'desc': Invoice, Debit Memo, Credit Memo, etc. (Z-A)
*/
...
-- Add index for type sorting performance
CREATE INDEX IF NOT EXISTS idx_invoices_type_date 
  ON acumatica_invoices(type, date DESC) WHERE type IS NOT NULL;

COMMENT ON FUNCTION search_invoices_paginated IS
  'Returns paginated invoice search results (50 at a time) with all filters and sorting on server side. Supports sorting by type, status, color, date, amount, balance, and customer name.';
