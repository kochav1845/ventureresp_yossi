/*
  # Fix Collector Progress Tracking Function
  
  1. Problem
    - no_change_count only counted on assignment date
    - Should show running count of invoices still with no status change
  
  2. Solution
    - Properly track closed invoices on the date they closed
    - Track red status changes on the date they changed
    - Show cumulative count of invoices assigned but with no status change up to each date
*/
