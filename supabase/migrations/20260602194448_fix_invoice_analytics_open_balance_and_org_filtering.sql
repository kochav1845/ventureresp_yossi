/*
  # Fix Invoice Analytics open balance and org filtering

  1. Problem
    - `total_open_balance` in cached analytics only counts status='Open'
      but should include Balanced and Credit Hold (real outstanding debt)
    - `open_invoice_balance` should include Credit Hold status
    - `get_filtered_invoice_aggregates` function has no organization filtering
    - The refresh function correctly includes all types (Invoice, Debit Memo, Credit Memo)
      and all relevant statuses (Balanced, Credit Hold, Open, Closed, Voided, Canceled)

  2. Changes
    - Update `refresh_cached_invoice_analytics_for_org` to calculate
      `total_open_balance` as sum of balance for Open + Balanced + Credit Hold
    - Update open_invoice_balance to also include Credit Hold status
    - Update open_cm_balance to include Credit Hold status
    - Update `get_filtered_invoice_aggregates` with same fixes + org filtering
    - The `total_amount` (gross amount) already includes all types and statuses

  3. Important Notes
    - After this migration, the user should refresh their analytics cache
    - Org filtering on get_filtered_invoice_aggregates uses get_user_org_id()
    - Both monthly and yearly views will now show accurate open balance figures
*/

-- Fix the org-scoped refresh function
...
-- Fix filtered aggregates function with org filtering + correct open balance
