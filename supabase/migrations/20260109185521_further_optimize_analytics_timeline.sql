/*
  # Further Optimize Customer Analytics Timeline Function

  1. Changes
    - Pre-aggregate all data before joining
...
BEGIN
  ...
  -- Get initial balance (all invoices before start date minus all payments before start date)
  initial_balance AS (
    SELECT
      COALESCE((SELECT SUM(balance) FROM acumatica_invoices WHERE date < v_date_from), 0) -
      COALESCE((SELECT SUM(payment_amount) FROM acumatica_payments WHERE application_date < v_date_from), 0) as starting_balance
  ),
  -- Combine all data
  combined_data AS (
    SELECT
      ds.period_date,
      COALESCE(i.invoice_count, 0) as invoices_opened,
      COALESCE(i.invoice_total, 0) as invoice_amount,
      COALESCE(p.payment_count, 0) as payments_made,
      COALESCE(p.payment_total, 0) as payment_amount
    FROM date_series ds
  ...
  SELECT
    cd.period_date,
    cd.invoices_opened::BIGINT,
    cd.invoice_amount::NUMERIC,
    cd.payments_made::BIGINT,
    cd.payment_amount::NUMERIC,
    (
      (SELECT starting_balance FROM initial_balance) + 
      SUM(cd.invoice_amount - cd.payment_amount) OVER (ORDER BY cd.period_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
