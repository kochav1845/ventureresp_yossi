/*
  # Optimize invoice breakdown by date function

  1. Problem
    - The function uses EXTRACT(YEAR/MONTH FROM date) which prevents index usage
    - Causes full table scan and statement timeout on large tables

  2. Fix
    - Rewrite WHERE clause to use a date range (>= first day, < first day of next month)
    - This allows PostgreSQL to use the existing btree index on the `date` column
    - Same result, dramatically faster execution
*/
