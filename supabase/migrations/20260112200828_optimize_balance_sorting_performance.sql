/*
  # Optimize Balance Sorting Performance

  1. Problem
    - Sorting by balance times out on large datasets
    - Existing index only covers balance > 0
    - Full table scan required for complete balance sort
    
  2. Solution
    - Add full balance index (including balance = 0)
    - Add query timeout to fail faster
    - Use NULLS LAST optimization
    
  3. Changes
    - Create comprehensive balance index
    - Add 10-second timeout to search function
    - Optimize sort order expressions
*/

-- Drop old partial index and create full index for balance sorting
DROP INDEX IF EXISTS idx_invoices_balance_date;

-- Create index that covers ALL balances (including 0) for sorting
CREATE INDEX IF NOT EXISTS idx_invoices_balance_sort 
  ON acumatica_invoices(balance DESC NULLS LAST, date DESC);

-- Create ascending balance index as well
CREATE INDEX IF NOT EXISTS idx_invoices_balance_sort_asc 
  ON acumatica_invoices(balance ASC NULLS LAST, date DESC);

-- Add index for common balance queries (unpaid invoices)
CREATE INDEX IF NOT EXISTS idx_invoices_unpaid_balance
  ON acumatica_invoices(balance, date DESC) WHERE balance > 0;

-- Update search function with timeout and optimizations
...
COMMENT ON FUNCTION search_invoices_paginated IS
  'Returns paginated invoice search results (50 at a time) with all filters and sorting on server side. 10-second timeout. Supports sorting by type, status, color, date, amount, balance, and customer name.';
