/*
  # Optimize get_customers_with_balance Performance
  
  This migration dramatically improves the performance of the get_customers_with_balance function
  by deferring expensive calculations and using more efficient query patterns.
  
  ## Changes
  
  1. Use LATERAL join to calculate avg_days_to_collect only for filtered customers
  2. Simplify date filtering logic to avoid multiple UNIONs
  3. Move expensive calculations after filtering
  4. Add query hints for better query planning
*/
...
BEGIN
  ...
  WITH customer_balances AS (
    SELECT
      ...
      -- Check if customer passes date filter (avoid separate CTE)
      BOOL_OR(
        CASE
  ...
  SELECT
    ...
    -- Calculate avg_days_to_collect only for returned customers using LATERAL
    (
      SELECT AVG(
      ...
      WHERE pia.amount_paid > 0
        ...
        AND p.customer_id = fc.customer_id
