/*
  # Add Test Customer Filter to Balance Function

  Updates the get_customers_with_balance function to support filtering
  test customers separately from real Acumatica customers.

  1. Modified Functions
    - `get_customers_with_balance` - Added `p_test_customers` parameter (boolean, default false)
      - When false (default): shows only real customers (existing behavior preserved)
      - When true: shows only test customers from the email system

  2. Important Notes
    - Default behavior unchanged - real customers only
    - Test customers accessible via explicit parameter
*/
