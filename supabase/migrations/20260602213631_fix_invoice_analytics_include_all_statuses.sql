/*
  # Fix Invoice Analytics to include all statuses (match Invoice Breakdown)

  1. Problem
    - The Invoice Breakdown (materialized view) shows 2,259 docs for Dec 2025 (includes On Hold)
    - The Invoice Analytics cached refresh function filters `status IN ('Balanced', 'Credit Hold', 'Open', 'Closed', 'Voided', 'Canceled')`
      which gives only 2,258 (misses the 1 On Hold invoice)
    - This causes totals to not match between the two pages
    - The cache was also stale (calculated May 27 with an old function)

  2. Changes
    - Remove restrictive status filter from `refresh_cached_invoice_analytics_for_org`
      so ALL statuses are included in total counts and amounts (matching the MV)
    - Keep open balance calculations limited to Open/Balanced/Credit Hold (those are the only
      statuses that represent outstanding debt)
    - Same fix for `get_filtered_invoice_aggregates`
    - Both functions still filter types to Invoice/Debit Memo/Credit Memo

  3. Notes
    - After this migration, the cache must be refreshed to see correct data
    - The Invoice Breakdown MV includes all statuses in its totals - analytics should match
    - On Hold invoices count in totals but NOT in open balance
*/

-- Fix the org-scoped refresh function - remove status restriction on totals
...
-- Fix filtered aggregates function - remove status restriction on totals
