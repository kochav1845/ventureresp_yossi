/*
  # Fix Color Status Sort Parameter Name

  1. Updates
    - Update sort_by checks to accept 'color_status' instead of 'color'
    - Frontend sends 'color_status', backend now accepts it
*/
...
BEGIN
  ...
  IF search_term IS NOT NULL AND search_term != '' THEN
    ...
    ORDER BY
      ...
      CASE WHEN sort_by IN ('color', 'color_status') AND sort_order = 'desc' THEN
        CASE WHEN i.color_status IS NULL OR i.color_status = 'none' THEN 1 ELSE 0 END
      END ASC,
      CASE WHEN sort_by IN ('color', 'color_status') AND sort_order = 'asc' THEN
        CASE WHEN i.color_status IS NULL OR i.color_status = 'none' THEN 0 ELSE 1 END
      END ASC,
      CASE WHEN sort_by IN ('color', 'color_status') AND sort_order = 'desc' THEN i.color_status END DESC NULLS LAST,
      CASE WHEN sort_by IN ('color', 'color_status') AND sort_order = 'asc' THEN i.color_status END ASC NULLS LAST,
