/*
  ...
  The trigger uses ON CONFLICT (id) but user_profiles also has a UNIQUE constraint
  on email. If a profile already exists with the same email (e.g., from a previous
  deleted auth user or pending_users flow), the insert fails.

  Fix: Use ON CONFLICT (email) as fallback, or delete the stale profile first.
*/
...
BEGIN
  ...
  -- Remove any stale profile with same email but different id
