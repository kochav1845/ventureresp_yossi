/*
  # Fix Analytics Date Filtering Logic

  1. Issue
    - Analytics function always uses INNER JOIN with date_filtered_customers
    - This excludes customers when no date filters are set
    - Customer list uses conditional logic: include ALL when no dates set

  2. Changes
    - Change INNER JOIN to LEFT JOIN
    - Add conditional WHERE clause to match customer list logic
    - When p_date_from and p_date_to are both NULL, include ALL customers
*/
