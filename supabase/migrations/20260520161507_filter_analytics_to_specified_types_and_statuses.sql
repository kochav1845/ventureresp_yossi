/*
  # Filter invoice analytics to specified types and statuses only

  1. Changes
    - Refresh function now filters to ONLY include:
      - Types: Invoice, Debit Memo, Credit Memo
      - Statuses: Balanced, Credit Hold, Open, Closed, Voided, Canceled
    - Excludes: On Hold, Scheduled statuses and Credit WO type
    - Same logic applied to get_filtered_invoice_aggregates

  2. Notes
    - Previously only excluded On Hold status
    - Now also excludes Scheduled status and Credit WO type
    - This aligns our totals with what Acumatica shows
*/
...
BEGIN
  ...
  IF p_period_type = 'daily' THEN
    ...
    main_agg AS (
      SELECT
        ...
        COALESCE(SUM(CASE WHEN type = 'Debit Memo' AND status IN ('Open', 'Balanced') THEN balance ELSE 0 END), 0) AS open_dm_balance,
        COUNT(CASE WHEN type = 'Debit Memo' AND status IN ('Open', 'Balanced') THEN 1 END)::integer AS open_dm_count
      FROM base
      GROUP BY inv_date
    ),
    type_agg AS (
  ...
  ELSIF p_period_type = 'monthly' THEN
    ...
    SELECT
      ...
      COALESCE(SUM(CASE WHEN i.type = 'Debit Memo' AND i.status IN ('Open', 'Balanced') THEN i.balance ELSE 0 END), 0),
      COUNT(CASE WHEN i.type = 'Debit Memo' AND i.status IN ('Open', 'Balanced') THEN 1 END)::integer,
      now(), now()
    FROM acumatica_invoices i
...
-- Update filtered aggregates function with same filter criteria
...
AS $$
  WITH filtered_invoices AS (
    ...
    FROM acumatica_invoices i
    WHERE
      i.type IN ('Invoice', 'Debit Memo', 'Credit Memo')
      AND i.status IN ('Balanced', 'Credit Hold', 'Open', 'Closed', 'Voided', 'Canceled')
