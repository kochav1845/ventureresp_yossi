/*
  # Create Process Auto Ticket Rules Function

  1. Creates a PostgreSQL function that processes auto-ticket rules
  2. This function runs directly in the database without needing edge functions
  3. Replaces the need for external HTTP calls in the cron job
  
  Rules:
  - invoice_age: Creates tickets for invoices within a specific age range
  - payment_recency: Creates tickets when customer hasn't paid within a time range
*/
    ...
    BEGIN
      ...
      IF FOUND THEN
        ...
        IF v_new_invoice_refs IS NOT NULL AND array_length(v_new_invoice_refs, 1) > 0 THEN
          ...
          VALUES (
            ...
            'invoice_added',
            'Auto-rule added ' || array_length(v_new_invoice_refs, 1) || ' invoice(s)'
          );
      ...
      ELSE
        ...
        VALUES (
          ...
          v_rule.assigned_collector_id,
          'created',
          'Auto-created ticket with ' || array_length(v_invoice_refs, 1) || ' invoice(s)'
        );
