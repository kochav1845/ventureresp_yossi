/*
  # Fix Customer Filters: Invoice Amount and Days Overdue

  1. Changes
    - `p_min_invoice_amount` / `p_max_invoice_amount` now filter on `i.amount` (original invoice amount) instead of `i.balance`
    - Invoice-level filters (amount, days overdue, date) restrict which invoices contribute to filtered_gross_bal, filtered_inv_count
    - Customers only appear if they have at least one invoice matching invoice-level filters
    - Customer-level filters (balance, invoice count) remain unchanged

  2. Important Notes
    - Dropped and recreated get_customers_with_balance_count to update signature
*/

-- Drop the old count function to recreate with updated logic
DROP FUNCTION IF EXISTS public.get_customers_with_balance_count(text,text,text,timestamp with time zone,timestamp with time zone,text,numeric,numeric,integer,integer,numeric,numeric,boolean,text,integer,integer,boolean);

CREATE OR REPLACE FUNCTION public.get_customers_with_balance(
  p_search text, p_status_filter text, p_country_filter text,
  p_sort_by text, p_sort_order text, p_limit integer, p_offset integer,
  p_date_from timestamp with time zone, p_date_to timestamp with time zone,
  p_balance_filter text, p_min_balance numeric, p_max_balance numeric,
  p_min_open_invoices integer, p_max_open_invoices integer,
  p_min_invoice_amount numeric, p_max_invoice_amount numeric,
  p_exclude_credit_memos boolean, p_date_context text,
  p_calculate_avg_days boolean, p_min_days_overdue integer, p_max_days_overdue integer,
...
RETURNS TABLE(
  ...
  statement_cycle text, parent_account text, price_class text,
  shipping_terms text, acumatica_record_id text,
...
CREATE OR REPLACE FUNCTION public.get_customers_with_balance_count(
  ...
  p_min_open_invoices integer DEFAULT NULL,
  p_max_open_invoices integer DEFAULT NULL,
    ...
    SELECT
      ...
      CASE WHEN v_has_filter THEN
        COUNT(*) FILTER (WHERE i.type IN ('Invoice', 'Debit Memo')
          ...
          AND (
            (p_min_days_overdue IS NULL AND p_max_days_overdue IS NULL)
            OR (i.date IS NOT NULL AND GREATEST(0, (CURRENT_DATE - i.date)::INT) >= COALESCE(p_min_days_overdue, 0) AND GREATEST(0, (CURRENT_DATE - i.date)::INT) <= COALESCE(p_max_days_overdue, 999999))
          )
          AND (p_min_invoice_amount IS NULL OR i.amount >= p_min_invoice_amount)
          AND (p_max_invoice_amount IS NULL OR i.amount <= p_max_invoice_amount)
        )
