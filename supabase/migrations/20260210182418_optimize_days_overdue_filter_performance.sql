/*
  # Optimize Days Overdue Filter Performance

  1. Issue
    - Query is timing out after adding days overdue parameters
    - Need to optimize filtering logic to prevent slow queries

  2. Changes
    - Move days overdue filtering earlier in the CTE chain
    - Only calculate max_overdue_days when actually needed for filtering
    - Add early exit conditions for better performance

  3. Impact
    - Faster query execution
    - No timeouts on customer list view
*/
...
BEGIN
  ...
  WITH customer_balances AS (
    ...
    GROUP BY i.customer
    -- Apply days overdue filter early to reduce dataset
    HAVING (p_min_days_overdue IS NULL OR MAX(
        CASE
          WHEN i.date IS NOT NULL AND i.balance > 0 AND i.type = 'Invoice'
          THEN GREATEST(0, (CURRENT_DATE - i.date)::INT)
          ELSE 0
      ...
      AND (p_max_days_overdue IS NULL OR MAX(
        CASE
          WHEN i.date IS NOT NULL AND i.balance > 0 AND i.type = 'Invoice'
          THEN GREATEST(0, (CURRENT_DATE - i.date)::INT)
          ELSE 0
  ...
  filtered_customers AS (
    ...
    WHERE
      ...
      -- Days overdue filtering is now in HAVING clause of customer_balances CTE
      AND (
...
COMMENT ON FUNCTION get_customers_with_balance IS 
  'Returns customers with outstanding balances. Days overdue calculated from invoice date. Optimized with early filtering in HAVING clause.';
