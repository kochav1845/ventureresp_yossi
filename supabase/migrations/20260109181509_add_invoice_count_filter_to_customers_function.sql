/*
  # Add Invoice Count Filter to get_customers_with_balance Function

  1. Changes
    - Adds p_min_open_invoices parameter for minimum invoice count filtering
    - Adds p_max_open_invoices parameter for maximum invoice count filtering
    - Applies invoice count filtering at database level for better performance

  2. Benefits
    - Completes the migration of all filters to database level
    - Improves performance by filtering before pagination
*/

-- Drop existing function
DROP FUNCTION IF EXISTS get_customers_with_balance(TEXT, TEXT, TEXT, TEXT, TEXT, INT, INT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, NUMERIC, NUMERIC);

-- Create updated version with invoice count filter parameters
