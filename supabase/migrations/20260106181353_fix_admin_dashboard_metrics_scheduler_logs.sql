/*
  # Fix Admin Dashboard Metrics Function - Remove scheduler_logs dependency

  1. Changes
    - Replace scheduler_logs with email_logs table that actually exists
    - Use email_logs to track email activity metrics
    - Maintain same metrics structure for backward compatibility

  2. Notes
    - scheduler_logs table never existed
    - email_logs table contains the actual email sending records
*/
...
BEGIN
  ...
  -- Email metrics (using email_logs instead of scheduler_logs)
  email_stats AS (
    SELECT
      COUNT(*) FILTER (WHERE status = 'sent') as total_sent,
      COUNT(*) FILTER (WHERE template_name ILIKE '%census%') as census_emails,
      COUNT(*) FILTER (WHERE template_name ILIKE '%report%') as report_emails
    FROM email_logs
