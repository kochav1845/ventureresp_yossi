...
async function processSync(jobId: string, startDate: string, endDate: string) {
  ...
  try {
    ...
    for (const payment of payments) {
      try {
        ...
        if (existing) {
          ...
          if (error) errors.push(`Update failed for ${refNbr}: ${error.message}`);
          else updated++;
        } else {
          const { error } = await supabase.from('acumatica_payments').insert(paymentData);
          if (error) errors.push(`Insert failed for ${refNbr}: ${error.message}`);
          else created++;
        }
      } catch (error: any) {
    ...
    await supabase.from('async_sync_jobs').update({
      ...
      progress: { created, updated, total: payments.length, errors: errors.slice(0, 10) }
    }).eq('id', jobId);
