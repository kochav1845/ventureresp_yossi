...
async function processSync(jobId: string, startDate: string, endDate: string) {
  ...
  try {
    ...
    // CRITICAL FIX: Use ApplicationDate instead of LastModifiedDateTime
    ...
    console.log(`Fetching payments with ApplicationDate between ${filterStartDate} and ${filterEndDate}`);

    const paymentsResponse = await sessionManager.makeAuthenticatedRequest(credentialsObj, paymentsUrl);

    if (!paymentsResponse.ok) {
      throw new Error(`Failed to fetch payments: ${await paymentsResponse.text()}`);
    }
      ...
      try {
        ...
        if (paymentDbId) {
          ...
          try {
            ...
            console.log(`[APP-HISTORY] Fetching ApplicationHistory for ${type} ${refNbr}`);
            ...
            if (detailResponse.ok) {
              ...
              if (detailData.files && Array.isArray(detailData.files) && detailData.files.length > 0) {
                console.log(`Processing ${detailData.files.length} files for payment ${refNbr}`);
          ...
          // Process ApplicationHistory
          if (applicationHistory.length > 0) {
            console.log(`Processing ${applicationHistory.length} applications for payment ${refNbr}`);
            ...
            for (const app of applicationHistory) {
              ...
              if (!invoiceExists && docType === 'Invoice') {
                ...
                if (!fetchResult.success) {
                  errors.push(`Invoice ${invoiceRefNbr} not found for payment ${refNbr}`);
                }
              }
