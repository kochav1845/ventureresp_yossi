...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const filters = [];
    if (customer) filters.push(`Customer eq '${customer}'`);
    if (amount) filters.push(`Amount eq ${amount}`);

    const filterString = filters.join(' and ');
    const url = `${acumaticaUrl}/entity/Default/24.200.001/Invoice?$filter=${filterString}&$select=ReferenceNbr,Date,Status,Amount,Balance,DueDate,Customer,CustomerName`;
    ...
    const result = {
      searchCriteria: { customer, amount },
      credentialsUsed: {
      ...
      rawData: !Array.isArray(data) ? data : undefined
    };
